...
function insertCustomer(PDO $pdo, $owner_name, $email, $contact_number, $num_pets_1, $num_pets_2) {
  try {
    ...
    WHERE c.CUSTOMER_EMAIL = :email AND a.SERVICE_ID = 2;
    ");
    ...
    FROM CUSTOMER c
    INNER JOIN APPOINTMENT a ON c.CUSTOMER_ID = a.CUSTOMER_ID
    WHERE c.CUSTOMER_CONTACTNUM = :contact_number AND a.SERVICE_ID = 2;
    ");
    ...
    $stmt = $pdo->prepare("
      ...
      FROM CUSTOMER c
      INNER JOIN APPOINTMENT a ON c.CUSTOMER_ID = a.CUSTOMER_ID
      WHERE c.CUSTOMER_NAME = :owner_name AND a.SERVICE_ID = 2;
    ");
...
function insertAppointment(PDO $pdo, $service_id, $customer_id, $appoint_status) {
  try {
    $stmt = $pdo->prepare("
      INSERT INTO APPOINTMENT (SERVICE_ID, CUSTOMER_ID, APPOINT_STATUS)
      VALUES (:service_id, :cus_id, :appoint_status)
    ");
...
if (empty($owner_name) || empty($email) || empty($contact_number) || !isset($num_pets_1) || !isset($num_pets_2)) {
  ...
  echo json_encode(array("success" => false, "message" => "Contact number must be 11 digits (no letters allowed)."));
  exit();
} else {
  try {
    ...
    if ($customerResult['success']) {
      ...
      $appointmentResult = insertAppointment($pdo, $service_id, $customer_id, $appoint_status);
