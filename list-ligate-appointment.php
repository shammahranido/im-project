...
try {
    ...
    if (isset($_GET['appoint_id'])) {
        ...
        if ($appointmentDetails['appoint_status'] == 'In Progress') {
            header('Location: inprogress-ligate.php');
          }
    ...
    $query = 'SELECT appointment.*, customer.customer_name, 
                customer.cus_num_dog, customer.cus_num_cat 
              FROM appointment 
              JOIN customer ON appointment.customer_id = customer.customer_id 
...
<body class="bg-gray-100">
    <div class="container mx-auto p-6">
        <h1 class="text-2xl font-bold mb-4">Ligate Appointments</h1>
        <!-- Search Forms -->
        <div class="mb-4">
            <form method="GET" action="admin-menu.php">
                <input type="hidden" name="service" value="list-ligate-appointment">
                <div class="flex items-center mb-4">
                    ...
                    <a href="admin-menu.php?service=list-ligate-appointment"><i class="fas fa-undo"></i></a>
                </div>
            </form>
                    ...
                    <?php else : ?>
                        <?php foreach ($appointments as $appointment) : ?>
                            <tr>
                                ...
                                <td class="pl-3">
                                    <a href="admin-frontend-ligate-view.php?appoint_id=<?=htmlspecialchars($appointment['appoint_id']) ?>"><i class="fa fa-eye" style="margin-right:15px;"></i></a>
                                    <a href="admin-frontend-ligate-edit.php?appoint_id=<?=htmlspecialchars($appointment['appoint_id']) ?>"><i class="fas fa-edit" style="color:green; margin-right:15px;"></i></a>
                                    <a href="list-ligate-appointment.php?appoint_id=<?= htmlspecialchars($appointment['appoint_id']) ?>" onclick="return confirm('Are you sure you want to remove the appointment?')"><i class="fas fa-trash" style="color:red;"></i></a>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    <?php endif; ?>
