    ...
    if (isset($_POST['name'], $_POST['email'], $_POST['contact'])) {
        ...
        if ($customer) {
            ...
            if ($appointment) {
                ...
                $_SESSION['error'] = "No 'In Progress' appointments found for the customer with service_id = 2.";
            }
        } else {
            $_SESSION['error'] = "No matching customer found with an appointment for service_id = 2.";
        }
    } else {
        $_SESSION['error'] = "Please provide name, email, and contact number.";
    }
    
    header('Location: customer-frontend-cancel-ligate.php');
    exit();
}
?>
...
<head>
    ...
    <title>Ligate Cancellation Appointment</title>
    <link href="https://cdn.jsdelivr.net/npm/tailwindcss@2.2.19/dist/tailwind.min.css" rel="stylesheet">
</head>
<body class="bg-red-200 flex items-center justify-center min-h-screen">
    <div class="bg-white p-6 rounded-lg shadow-lg w-full max-w-md">
        <div class="flex justify-between items-center mb-6">
            <h2 class="text-2xl font-bold text-gray-800">Ligate Cancellation Appointment</h2>
            <a href="index.php" class="font-bold text-gray-500 hover:text-gray-800">X</a>
        </div>
        <p class="text-gray-700">We're sorry to hear that you've decided to cancel your liagte vaccination appointment. We hope to assist you in the future. Thank you for your understanding.</p>
        <p class="text-xs text-red-500 mb-5">Make sure that the name, email, and contact number matches with the one you registered.</p>
        <form action="customer-frontend-cancel-ligate.php" method="post" class="space-y-4">
            <div>
